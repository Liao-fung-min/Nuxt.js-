<template lang="pug">
  <section>
    <div class="l-registered">
      <div class="col-container">
        <div class="col-row h-flex-lg-column h-pt-4 h-pb-4">
          <div class="l-registered__title h-flex h-align-items-center h-mb-lg-4"><img src="~/assets/img/logo-light.svg" width="40" height="40" />
            <h3 class="h-inline">訂閱你我的甜蜜郵件</h3>
          </div>
          <form class="l-registered__form h-flex h-align-content-between" action="/"><i class="fas fa-envelope l-registered__form__icon"></i>
            <input class="l-registered__form__text" type="text" />
            <button class="l-registered__form__submit h-ml-auto"><i class="fas fa-arrow-right"></i></button>
          </form>
        </div>
      </div>
    </div>
    <div class="l-footer">
      <div class="col-container">
        <div class="col-row h-mb-4">
          <div class="l-footer__contact">
            <div class="l-footer__contact__logo h-mb-md-4"><img src="~/assets/img/logotype-lg-dark.svg" width="171" height="26" /></div>
            <ul class="l-footer__contact__content">
              <li><a href="tel:+0712345678"></a>07-1234-5678</li>
              <li><a href="mailto:sweetaste@email.com"></a>sweetaste@email.com</li>
              <li><a href="https://www.google.com.tw/maps/place/800%E9%AB%98%E9%9B%84%E5%B8%82%E6%96%B0%E8%88%88%E5%8D%80/@22.6284345,120.2987625" target="_blank">800 高雄市新興區幸福路 520 號</a></li>
            </ul>
          </div>
          <div class="l-footer__title h-md-none"><img src="~/assets/img/footer__title.svg" witdt="59" height="218" /></div>
        </div>
        <div class="col-row h-align-items-end h-align-items-md-start h-flex-md-column">
          <ul class="l-footer__social h-flex h-mb-md-3">
            <li><a href="#"><img src="~/assets/img/ic-line@.svg" width="32" height="32" /></a></li>
            <li><a href="#"><img src="~/assets/img/ic-facebook.svg" width="32" height="32" /></a></li>
          </ul>
          <div class="l-footer__copyright">© 2018 Sweetaste* All Rights Reserved</div>
        </div>
      </div>
    </div>
  </section>
</template>
  